#!/usr/bin/env python3
"""
Rebuild the GPS-index input CSVs from the gzipped EXIF dumps for a city.

Parses every ``data/{city}/exif/*_exif.txt.gz`` and writes the two files that
...
def exif_video_id(exif_file: Path) -> str:
    """``day_1_..._5856a1d1_exif.txt.gz`` -> ``day_1_..._5856a1d1``."""
    return exif_file.name.removesuffix(".gz").removesuffix(".txt").removesuffix("_exif")


def read_exif_text(exif_file: Path) -> str:
    with gzip.open(exif_file, "rt", encoding="utf-8", errors="ignore") as f:
        return f.read()


def parse_video_metadata_from_exif(exif_content: str, exif_file: Path) -> Optional[Dict]:
    """Parse video metadata from EXIF content."""
    video_id = exif_video_id(exif_file)
...
def build_frame_metadata(frames_dir: Path, exif_files: List[Path]) -> List[Dict]:
    ...
    for exif_file in exif_files:
        video_id = exif_video_id(exif_file)
        content = read_exif_text(exif_file)
...
def rebuild_csvs_from_exif(city: str, project_root: Path) -> int:
    """Parse ``data/{city}/exif/*_exif.txt.gz`` into the GPS-index input CSVs."""
    ...
    exif_files = sorted(exif_dir.glob("*_exif.txt.gz"))
