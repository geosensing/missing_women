...
Inputs:
    - EXIF directory: gzipped exiftool text (*_exif.txt.gz) from 00_process_videos.py
...
def parse_exif_for_faces(exif_path: Path) -> list[dict]:
    ...
    with gzip.open(exif_path, "rt", encoding="utf-8", errors="ignore") as f:
        for line in f:
...
def parse_exif_filename(exif_path: Path) -> tuple[str, str, str]:
    ...
    EXIF filename formats:
        Old: day_1_4_28_2026_11.15_5856a1d1_exif.txt.gz (day_NUM_MONTH_DATE_YEAR)
        New: day_13_6_2026_GX012552_95e48ead_exif.txt.gz (day_NUM_MONTH_YEAR)
    Returns: (day_folder, video_name, frame_id)

    frame_id is without hash to avoid duplicates from same video.
    """
    name = exif_path.name.removesuffix(".gz").removesuffix(".txt")
...
def main():
    ...
    parser.add_argument(
        "-e", "--exif-dir", required=True, help="Directory of gzipped EXIF dumps (*_exif.txt.gz)"
    )
    parser.add_argument(
