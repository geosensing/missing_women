...
def make_fig2_distribution(df: pd.DataFrame, cities: list[str]) -> None:
    ...
    fig, axes = plt.subplots(
        nrows,
        ncols,
        figsize=(6.5, 3.8 if nrows > 1 else 2.2),
        sharex=True,
        sharey=True,
        squeeze=False,
    )
    ...
    for ax, city in zip(axes, cities):
        ...
        ax.text(
            0.97,
            0.95,
            f"Person-weighted {s['weighted']:.1%}\nImage-level mean {s['unweighted']:.1%}",
...
def style_share_axis(ax, *, show_xlabel: bool = True, show_parity: bool = False) -> None:
...
def make_fig3_multipanel(df: pd.DataFrame, cities: list[str]) -> None:
    ...
    # Panel A: two directly labeled estimates per city.
...
def make_figS3_context_patterns(df: pd.DataFrame, cities: list[str]) -> None:
    ...
    # Panel A: pooled POI comparisons; close estimates use a fixed label column.
    ax = fig.add_subplot(outer[0])
    ...
    for row, (field, label) in enumerate(pois):
        ...
        for offset, name, summary, color in [
            (-0.10, "Visible", present, "0.05"),
            (0.10, "Not visible", absent, "0.45"),
        ]:
            y = row + offset
