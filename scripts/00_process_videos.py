...
1. Extract EXIF (if not exists) → save gzipped .txt.gz
...
def parse_video_metadata(exif_content: str, video_path: Path, video_id: str) -> Dict:
    ...
    return {
        ...
        "exif_filename": f"{video_id}_exif.txt.gz",
    }
...
def process_video(
    ...
    exif_file = exif_dir / f"{video_id}_exif.txt.gz"
    legacy_exif_file = exif_dir / f"{video_id}_exif.txt"

    if exif_file.exists():
        with gzip.open(exif_file, "rt", encoding="utf-8") as f:
            exif_content = f.read()
    elif legacy_exif_file.exists():
        exif_content = legacy_exif_file.read_text(encoding="utf-8")
    else:
        exif_content = run_exiftool(video_path, timeout)
        if exif_content is None:
            return None, [], []
        with gzip.open(exif_file, "wt", encoding="utf-8") as f:
            f.write(exif_content)
