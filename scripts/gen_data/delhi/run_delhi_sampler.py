#!/usr/bin/env python3
"""
Delhi Data Collection Sampler

This script generates optimized data collection itineraries for Delhi by:
1. Extracting NCT of Delhi boundary from OpenStreetMap
...
OUTPUT_DIR = PROJECT_ROOT / "sampling" / "delhi"
...
def check_step_complete(step_name):
    ...
    checks = {
        "boundary": Path(f"{OUTPUT_DIR}/boundaries/delhi_boundary.geojson").exists(),
...
def main():
    parser = argparse.ArgumentParser(description="Delhi Data Collection Sampler")
    parser.add_argument(
    ...
    print("=" * 50)
    print("Delhi Data Collection Sampler")
    print("=" * 50)

    # 1. Extract Delhi Boundary from OSM
    if args.resume and check_step_complete("boundary"):
        print("\n1. Loading existing Delhi boundary...")
        boundary_gdf = gpd.read_file(f"{OUTPUT_DIR}/boundaries/delhi_boundary.geojson")
        delhi_boundary = boundary_gdf.geometry.union_all()
    else:
        print("\n1. Extracting NCT of Delhi boundary from OpenStreetMap...")
        gdf_admin = ox.features_from_place("Delhi, India", tags={"boundary": "administrative"})
        delhi_gdf = gdf_admin[(gdf_admin["name"] == "Delhi") & (gdf_admin["admin_level"] == "4")]

        if len(delhi_gdf) == 0:
            raise ValueError("Could not find NCT of Delhi boundary in OSM data")

        delhi_boundary = delhi_gdf.geometry.union_all()

        boundary_gdf = gpd.GeoDataFrame(geometry=[delhi_boundary], crs="EPSG:4326")
        boundary_gdf.to_file(f"{OUTPUT_DIR}/boundaries/delhi_boundary.geojson", driver="GeoJSON")

        _, ax = plt.subplots(figsize=(10, 10))
        delhi_gdf.plot(ax=ax, facecolor="lightblue", edgecolor="blue", linewidth=2)
        ax.set_title("NCT of Delhi Boundary")
        plt.tight_layout()
        plt.savefig(f"{OUTPUT_DIR}/visualizations/boundary.png", dpi=150)
        plt.close()

        print(f"   Boundary bounds: {delhi_boundary.bounds}")
    ...
    if args.resume and check_step_complete("network"):
        ...
        G = ox.graph_from_polygon(delhi_boundary, network_type="drive")
        ...
        ax.set_title("Delhi Street Network")
        plt.tight_layout()
    ...
    create_folium_map(all_itineraries, boundary_gdf, delhi_boundary, OUTPUT_DIR, "Delhi")

    print("\n" + "=" * 50)
    print("SUMMARY")
    print("=" * 50)
    print(f"Boundary: {OUTPUT_DIR}/boundaries/delhi_boundary.geojson")
