...
def compute_frame_datetime(
    ...
    align with the embedded GPS timeline, so ``gps`` is correct; ``recording`` is
    unreliable (its metadata timestamp can sit well inside the GPS log, e.g. mumbai
    day-10, or minutes off, e.g. day-11). Each falls back to the other when its
